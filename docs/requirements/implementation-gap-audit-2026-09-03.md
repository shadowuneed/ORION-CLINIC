...
- Phase 2I.1: шесть вспомогательных операций анализа/STT переведены на точное
  назначение врача; 467 тестов и полная локальная проверка прошли. Основные
  `/api/workspace`, интерфейс выбора назначения и атрибуция записей БД ещё требуют
  Phase 2I.2. Полный список — `phase-2i-encounter-access.md`.
