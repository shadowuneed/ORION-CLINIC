...
- Phase 2I.1–2I.2: вспомогательные операции анализа/STT и основные
  `/api/workspace` проверяют точное назначение врача. Выбор назначения,
  режим просмотра и передача контекста между дэшбордом и `/live` реализованы.
  Атрибуция команд/событий/сессий в БД, новые SQL-ограничения и проверка отзыва
  прав перед повтором команды/сохранением долгого ответа остаются в Phase 2I.3.
  Полная Phase 2I НЕ завершена. Доказательства — Last handoff в MASTER_PLAN;
  полный список — `phase-2i-encounter-access.md`.
