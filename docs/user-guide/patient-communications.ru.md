...
- каждый API-запрос повторно проверяет Sites identity, конкретное действующее
  назначение сотрудника в отделение, право `communications.manage`, организацию,
  филиал, роль и доступ к точному источнику.

Если у сотрудника несколько подходящих назначений, сначала выберите **«Рабочее
назначение»**. Даже два назначения в одном филиале не объединяют права.
Выбор сохраняется в адресе страницы. Во время сохранения и при открытой форме
переключение назначения заблокировано, чтобы действие не попало в другой контекст.
Недоступное или отозванное назначение не заменяется другим автоматически:
заново откройте раздел через основное меню и выберите доступное назначение.
Все новые действия сохраняют его идентификатор в БД и аудите.
...
Последняя команда допускает повторный запуск. Она добавляет явное тестовое
назначение медсестры в отделение (после care seed), а также одну локальную
